...
while true; do
    ...
    TARGET_EVE_DIR="$BASE_EVE_DIR/Frontier"
...
# 4. Pull Frontier Remote Binary Payload
DOWNLOAD_DIR="$EVE_DIR/drive_c/users/Public/Downloads"
cd "$DOWNLOAD_DIR"

echo "Fetching target distribution package..."
wget -q --show-progress -U "Mozilla/5.0" -O eve-frontier-setup.exe "https://ccpgames.com/eve-frontier/frontier/win32/x64/eve-frontier-latest%20Setup.exe"
...
7z x -y eve-frontier-setup.exe -o"$LAUNCHER_DIR" > /dev/null

echo "Scanning for nested payload dependencies..."
cd "$LAUNCHER_DIR"
find . -type f -name "*.nupkg" -exec 7z x -y {} -o"$LAUNCHER_DIR" \; > /dev/null

if [ ! -f "$LAUNCHER_DIR/lib/net45/eve-frontier.exe" ]; then
    echo -e "${RED}ERROR: File integrity check failed! 'eve-frontier.exe' missing.${NC}"
    exit 1
fi
chmod +x "$LAUNCHER_DIR/lib/net45/eve-frontier.exe"
...
7z x -y "$DOWNLOAD_DIR/eve-frontier-setup.exe" -o"$ICON_DIR" "\$PLUGINSDIR/installer_icon.ico" > /dev/null 2>&1 || true

if [ ! -f "$ICON_DIR/\$PLUGINSDIR/installer_icon.ico" ]; then
    7z x -y "$LAUNCHER_DIR/lib/net45/eve-frontier.exe" -o"$ICON_DIR" "app.ico" > /dev/null 2>&1 || true
...
SHORTCUT_PATH="$HOME/Desktop/EVE-Frontier.desktop"
echo "Creating application shortcut entry: $SHORTCUT_PATH"

cat << EOF > "$SHORTCUT_PATH"
[Desktop Entry]
Name=EVE Frontier
Comment=Launch EVE Frontier via Extracted Wine Runtime
Exec=env WINEPREFIX="$EVE_DIR" DXVK_STATE_CACHE_DIR="$DX11_CACHE" VKD3D_SHADER_CACHE_DIR="$DX12_CACHE" MESA_GL_CACHE_DIR="$MESA_CACHE" wine "$LAUNCHER_DIR/lib/net45/eve-frontier.exe" --disable-gpu-sandbox --no-sandbox
Terminal=false
Type=Application
Categories=Game;
Icon=EA28_EVE Frontier.0
EOF

chmod +x "$SHORTCUT_PATH"
echo "Done! Eve Frontier is installed, and The shortcut is ready on your desktop."
